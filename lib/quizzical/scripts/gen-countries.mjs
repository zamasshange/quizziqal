const list = [
  ...
  ["Palestine", "ps"], ["Vatican City", "va"], ["Kosovo", "xk"],
];

import fs from "fs";
import path from "path";

const out = `// ${list.length} sovereign states & territories — used for the Flags quiz pool.
export type CountryRef = { name: string; iso2: string };

export const ALL_COUNTRIES: CountryRef[] = ${JSON.stringify(
  list.map(([name, iso2]) => ({ name, iso2 })),
  null,
  2,
)};
...
fs.writeFileSync(
  path.join(process.cwd(), "lib", "allCountries.ts"),
  out,
  "utf8",
);
